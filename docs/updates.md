...
- May 29 Spark CLI r17 installer pin with hosted checksums, release manifest, command metadata, attestations, voice-safe setup defaults, stale Windows shim cleanup, production smoke checks, and a softer upgrade-refresh pause when existing secrets need a secure backend before rewrite.
