...
function extractScripts(html) {
  const scripts = [];
  const scriptStartRe = /<script\b([^>]*)>/gi;
  let match;
  while ((match = scriptStartRe.exec(html)) !== null) {
    const attrs = match[1] || "";
    const startIndex = scriptStartRe.lastIndex;
    ...
    while (index < html.length) {
      ...
      if (html.slice(index, index + 9).toLowerCase() === '</script>') {
        body = html.slice(startIndex, index);
        scriptStartRe.lastIndex = index + 9;
        foundEnd = true;
        break;
      }
      index++;
...
for (const file of files) {
  ...
  const foundScripts = extractScripts(html);
  for (const { attrs, body } of foundScripts) {
    const rel = file;
    if (getAttr(attrs, "src") !== null) continue;
